...
export const RoomObjects: React.FC<RoomObjectsProps> = ({
  ...
  const handlePointerOver = (label: string) => {
  ...
  useFrame((state, delta) => {
    ...
    // 10. Dynamic Canvas Rendering (Optimized & Throttled for 60-120fps performance)
    if (time - lastCanvasUpdateRef.current > 0.033) {
      lastCanvasUpdateRef.current = time;
      ...
      const { 
        ...
        ctxKeyboard, cKeyboard, texKeyboard
      } = canvasRefs;
      ...
      ctx1.fillRect(0, 0, c1.width, 32);

      // VS Code Tabs
      ctx1.fillStyle = '#38bdf8';
      ctx1.fillRect(16, 6, 120, 26);
      ctx1.fillStyle = '#000000';
      ctx1.font = 'bold 12px monospace';
      ctx1.fillText('Antigravity.ts', 28, 23);

      ctx1.fillStyle = '#334155';
      ctx1.fillRect(142, 6, 100, 26);
      ctx1.fillStyle = '#94a3b8';
      ctx1.fillText('Cluster.tsx', 154, 23);

      ctx1.font = '13px monospace';
      const visibleLines = Math.floor(time * 3) % (codeLines.length + 4);
      codeLines.forEach((line, idx) => {
        if (idx <= visibleLines) {
          ctx1.fillStyle = '#475569';
          ctx1.fillText((idx + 1).toString().padStart(2, '0'), 15, 60 + idx * 24);
          ctx1.fillStyle = line.c;
          ctx1.fillText(line.t, 45, 60 + idx * 24);
        }
      });
      tex1.needsUpdate = true;

      // Screen 2 (Matrix Rain)
      ctx2.fillStyle = 'rgba(2, 6, 12, 0.2)';
      ctx2.fillRect(0, 0, c2.width, c2.height);
      ctx2.fillStyle = '#22c55e';
      ctx2.font = '14px monospace';

      const chars = '01ROMAARTIKOVFULLSTACKENGINEDEVELOPOPERREACTTYPESCRIPTNODEPOSTGRESQL';
      for (let i = 0; i < matrixColumns.length; i++) {
        const char = chars[Math.floor(Math.random() * chars.length)];
        const x = i * 16;
        const y = matrixColumns[i] * 16;
        ctx2.fillText(char, x, y);
        if (y > c2.height && Math.random() > 0.975) {
          matrixColumns[i] = 0;
        }
        matrixColumns[i]++;
      }
      ...
      // Screen 3 (Telemetry Dashboard)
      ctx3.fillStyle = '#070c18';
      ctx3.fillRect(0, 0, c3.width, c3.height);

      ctx3.strokeStyle = '#1e293b';
      ctx3.lineWidth = 1;
      ctx3.strokeRect(10, 10, 300, 160);
      ctx3.strokeRect(320, 10, 310, 160);
      ctx3.strokeRect(10, 180, 620, 170);

      // Telemetry Metrics
      ctx3.fillStyle = '#38bdf8';
      ctx3.font = 'bold 14px monospace';
      ctx3.fillText('CPU LOAD: RTX 4090 TI [38°C]', 20, 35);
      ctx3.fillStyle = '#4ade80';
      ctx3.fillText('RAM USAGE: 14.2 GB / 64 GB DDR5', 330, 35);
      ctx3.fillStyle = '#fbbf24';
      ctx3.fillText('NETWORK LATENCY: 0.18ms • TAILSCALE MESH', 20, 205);
      ...
      for (let x = 20; x < 300; x += 10) {
        const y = 100 + Math.sin(time * 4 + x * 0.05) * 35;
      ...
      ctx3.beginPath();
      for (let x = 330; x < 620; x += 10) {
        const y = 100 + Math.cos(time * 3 + x * 0.05) * 30;
        if (x === 330) ctx3.moveTo(x, y);
        else ctx3.lineTo(x, y);
      }
      ctx3.stroke();
      tex3.needsUpdate = true;

      // Screen 4 (GitHub Terminal Log)
      ctx4.fillStyle = '#090d16';
      ctx4.fillRect(0, 0, c4.width, c4.height);
      ctx4.fillStyle = '#38bdf8';
      ctx4.font = 'bold 14px monospace';
      ctx4.fillText('roma-artikov@battlestation:~/portfolio$ git log --oneline -n 6', 20, 30);

      const commits = [
        'commit a1f902c — feat: hyper-realistic 3D PC Battlestation & custom liquid cooling',
  ...
  return (
    <group position={[0, -0.6, 0]}>
      ...
      <group 
        ...
        onPointerOver={() => handlePointerOver('✨ Cyber Neon Sign (Click to Toggle)')}
      ...
      <group position={[0, 0, -1.2]}>
        ...
        <group 
          ...
          onPointerOver={() => handlePointerOver('🔮 3D Hologram Projector (Click to Cycle)')}
          onPointerOut={handlePointerOut}
        >
        ...
        <group 
          ...
          onPointerOver={() => handlePointerOver('🖥️ Curved 4K Display (Click to Switch View)')}
          onPointerOut={handlePointerOut}
        >
          ...
          <group 
            ...
            onPointerOver={(e) => {
              e.stopPropagation();
              handlePointerOver(lampOn ? '💡 ScreenBar Lamp (Click to turn OFF)' : '💡 ScreenBar Lamp (Click to turn ON)');
            }}
            onPointerOut={handlePointerOut}
          >
            <mesh castShadow>
        ...
        <group 
          ...
          onPointerOver={() => handlePointerOver('⌨️ Custom 75% Keyboard (Click or Type on Real Keys)')}
          onPointerOut={handlePointerOut}
        >
        ...
        <group 
          ...
          onPointerOver={() => handlePointerOver('📱 OLED Smartphone (Click to View Alerts)')}
          onPointerOut={handlePointerOut}
        >
        ...
        <group 
          ...
          onPointerOver={() => handlePointerOver('☕ Steaming Espresso (Click to Sip)')}
          onPointerOut={handlePointerOut}
        >
        ...
        <group 
          ...
          onClick={onPcClick}
          onPointerOver={() => handlePointerOver('⚡ RTX 4090 Battlestation (Click to Cycle RGB)')}
          onPointerOut={handlePointerOut}
        >
      ...
      <group 
        ...
        onPointerOver={() => handlePointerOver('🪑 Ergonomic Swivel Chair (Click to Spin)')}
        onPointerOut={handlePointerOut}
      >
      ...
      <group 
        ...
        onPointerOver={() => handlePointerOver('🐱 Cyber Kitty (Click to Pet & Purr)')}
        onPointerOut={handlePointerOut}
      >
